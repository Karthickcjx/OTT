  ...
  return (
    <button
      ...
      className={cx(
        ...
        primary
          ? 'playnix-button-primary hover:scale-[1.02]'
          : tone === 'kids'
      ...
      <div
        className={cx(
          ...
          isLight
            ...
            : 'bg-[linear-gradient(90deg,rgba(11,11,15,0.97)_0%,rgba(11,11,15,0.78)_32%,rgba(11,11,15,0.2)_64%,rgba(11,11,15,0.72)_100%)]',
        )}
      />
      <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(11,11,15,0.05),rgba(11,11,15,0.16)_38%,rgba(11,11,15,0.92)_100%)]" />
      ...
      <div className="relative mx-auto flex min-h-[92vh] max-w-[1600px] items-end px-4 pb-24 pt-28 sm:px-6 lg:px-10">
        <div className="w-full max-w-4xl animate-[stream-fade-up_0.75s_ease-out]">
          <div className="flex flex-wrap items-center gap-3">
            <span
              className={cx(
                'inline-flex items-center gap-2 rounded-full border px-4 py-2 text-[11px] font-bold uppercase tracking-[0.22em]',
                isKidsMode
                  ? 'border-amber-300/25 bg-amber-300/14 text-amber-50'
                : isLight
                  ? 'border-slate-300 bg-white/70 text-slate-700'
                  : 'border-fuchsia-300/25 bg-fuchsia-300/12 text-fuchsia-50',
              )}
            >
              {isKidsMode ? <Sparkles className="h-3.5 w-3.5" /> : <Clapperboard className="h-3.5 w-3.5" />}
              {isSeries ? 'Playnix Series' : 'Playnix Featured'}
            </span>
